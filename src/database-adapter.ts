...
export function loadFromCache(db: any, maxAge: number = 30 * 60 * 1000): VideoItem[] {
...
export function saveToCache(db: any, videos: VideoItem[]): void {
  ...
  const transaction = db.transaction((videos: VideoItem[]) => {
    for (const video of videos) {
      const id = video.link.split('watch?v=')[1] || video.link.split('/shorts/')[1] || video.link;
      insert.run(id, video.title, video.channel, video.link, video.published.getTime(), video.isShort ? 1 : 0, now);
    }
  });
