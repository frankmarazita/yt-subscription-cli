...
import { groupVideosByDays, formatTimeAgo } from '../utils/dateUtils.js';
...
export function SimpleVideoList({ videos, onSelect, onExit, refreshing = false, lastUpdated, cacheAge = 0 }: SimpleVideoListProps) {
  ...
  return (
    <Box flexDirection="column" width={terminalWidth}>
      ...
      <Box paddingX={1} marginBottom={1} justifyContent="space-between">
        <Text color="cyan" bold>📺 YouTube Subscription CLI - Latest Videos</Text>
        <Box>
          {refreshing && (
            <Text color="yellow">
      ...
      {videoGroups.map((group, groupIndex) => (
        <Box key={`${group.label}-${groupIndex}`} flexDirection="column" marginBottom={2}>
          {/* Day Header */}
          <Box paddingX={1} marginBottom={1}>
            <Text color="cyan" bold>{group.label} ({group.videos.length} videos)</Text>
          </Box>
          ...
          {group.videos.map((video, videoIndex) => {
            ...
            return (
              <Box key={video.link} paddingX={2} marginBottom={1} width={terminalWidth}>
                <Box flexDirection="column" width="100%">
                  <Text>
                    <Text color="yellow">{channel}</Text> | {title} | <Text color="green">{timeAgo}</Text> | <Text color="magenta">{type}</Text>
                  </Text>
                  <Box marginLeft={2}>
      ...
      <Box paddingX={1} flexDirection="column">
        ...
        <Text color="gray">💡 Tip: Use Ctrl+Click or Cmd+Click to open video links in your browser</Text>
      </Box>
    </Box>
  );
}